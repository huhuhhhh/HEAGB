...
load("../netdisorder");
data = xlsread('../GB_fixedT/multiTemp.xlsx','Sheet1','A1:G28');
...
[xData, yData, zData] = prepareSurfaceData(Mn_comp, Temp, y_CoMn_ANN*100 );
ft = fittype( 'lowess' );
% Fit model to data.
...
xlabel('\it{x}\rm (at%)')
ylabel('\it{T}\rm (K)')
title('ANN \it{\Gamma}\rm_{Disorder}')
text(0.25,1250,50,'Mn\rightarrowCo','color','k')
...
print -dpng -r300 Co_Mn_Disdiagram_ANN
