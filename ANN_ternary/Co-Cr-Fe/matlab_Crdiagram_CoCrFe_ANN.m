...
comp_test(:,4)  = 0.6-comp_test(:,1)-comp_test(:,3); % Fe
comp_test(:,5)  = ones(n,1)*0.2; % Mn

%for i = 1:1:length(z)
n_test = 0;
for i = 1:1:size(comp_test,1)
    if comp_test(i,4) >= 0.05 && comp_test(i,4) <= 0.35
        n_test = n_test + 1;
...
data_CoCrFe(:,1)=ones(size(comp_final,1),1)*1000;
data_CoCrFe(:,2:6) = comp_final;


y_CoCrFe_ANN = net(data_CoCrFe');
data_CoCrFe_extract(:,1)=(data_CoCrFe(:,2));
data_CoCrFe_extract(:,2)=(data_CoCrFe(:,4));
data_CoCrFe_extract(:,3)=(data_CoCrFe(:,5));

for i=1:1:size(data_CoCrFe_extract,1)
    data_CoCrFe_norm(i,:) = data_CoCrFe_extract(i,:)/sum(data_CoCrFe_extract(i,:));
end
...
[h,hg,htick] = terplot;
[hcont,ccont,hcb]=tersurf(data_CoCrFe_norm(:,1),data_CoCrFe_norm(:,2),data_CoCrFe_norm(:,3),y_CoCrFe_ANN'*100);
hlabels=terlabel('Co','Cr','Fe');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
print -dpng -r300 CoCrFe_diagramCr_ANN_2
