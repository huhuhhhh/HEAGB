...
% Y is the targeting variable, which is Co segregation  
Y = data(:,13); 
...
Y_Co = (disorder-min_dis).*slope_beta.*(Tc_co-Temp) - Y_fit;

% Calculate RMSE between DBAM and MC/MD simulations
RMSE_Co = sqrt(mean((Co_adp - Y_Co).^2)); 
display(RMSE_Co)
...
plot(Co_adp*100,Y_Co*100,'o','markersize',8,'color',cocolor,'markerfacecolor',cocolor)
hold on
plot([-35 0],[-35 0],'--','linewidth',1.5,'color',[0.5 0.5 0.5])
axis([-35 0 -35 0])

% axis label
xlabel('MC/MD \it{\Gamma}\rm_{Co} (nm^{-2})')
ylabel('Fitted \it{\Gamma}\rm_{Co} (nm^{-2})')
xticks([-35 -28 -21 -14 -7 0])
yticks([-35 -28 -21 -14 -7 0])
